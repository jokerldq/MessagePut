...
class CommandHandler:
    ...
    def handle(self, session, line):
        ...
        try:
            ...
            line = ''
        # 通过协议代码执行相应的方法
        method = getattr(self, 'zyy_' + cmd, None)
        try:
            method(session, line)
        except TypeError:
...
class Room(CommandHandler):
    ...
    def zyy_logout(self, session, line):
        # 退出房间
        f1.write(time.ctime() + ':  ' + session.name + '退出系统\n')
        f1.flush()
        if self.sessions == '':
            f1.close()
        raise EndSession
...
class LoginRoom(Room):
    ...
    def zyy_login(self, session, line):
        # 用户登录逻辑
        name = line.strip()
        # 获取用户名称
        if not name:
        ...
        f1.write(time.ctime() + ':  ' + session.name + '登录系统\n')
        f1.flush()
...
class ChatRoom(Room):
    ...
    def zyy_say(self, session, line):
        # 客户端发送消息
        print(line)
        self.broadcast(('time:' + time.strftime('%H:%M:%S', time.localtime(time.time())) + '\n' + session.name + ': ' + line + '\n').encode("utf-8"))
        re = (str(line),)
        conn = sqlite3.connect('用户.db')
        conn.execute('insert into message1(message) values(?)', re)
        conn.commit()
        conn.close()
        f1.write(time.ctime() + ':  ' + session.name + '发出消息： ' + line + '\n')
        f1.flush()

    def zyy_look(self, session, line):
        # 查看在线用户
        session.push(b'Online Users:   \n')
        for other in self.sessions:
            session.push((other.name + '\n').encode("utf-8"))
        f1.write(time.ctime() + ':  ' + session.name + '查看在线用户\n')
        f1.flush()
